...
Output:
    notebook/images/<image_name>/
        ...
        _objects.json           <- [{index, label, color_hex}, ...] — same per-index
                                   colour save_preview draws with, machine-readable
                                   (added for suburb-historian's chat-driven 3D scene
                                   editor, Phase 0 of that plan)
...
# Deterministic per-index colour palette — shared by save_preview's overlay/
# legend and save_output's _objects.json, so a segment's colour in the
# preview image and its color_hex in the machine-readable file are always
# the same value, not two independently-seeded draws that happen to agree
# today only because they both hardcode seed 42. Factored out 2026-09-02
# (suburb-historian's chat-driven 3D scene editor, Phase 0) — previously
# save_preview alone owned this via an inline `np.random.seed(42)`.
def _segment_colors(n: int) -> np.ndarray:
    np.random.seed(42)
    return np.random.rand(n, 3)


def _color_hex(rgb: np.ndarray) -> str:
    """rgb: a (3,) float array in [0, 1] (one row of _segment_colors' output) -> '#rrggbb'."""
...
def save_output(image_path: str, image_rgb: np.ndarray,
    ...
    for idx, seg in enumerate(segments):
        ...
        objects.append({"index": idx, "label": seg["label"], "color_hex": _color_hex(colors[idx])})
    ...
    objects_path = os.path.join(output_dir, "_objects.json")
